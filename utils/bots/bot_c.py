...
from ..game import Bot, PlayerPerspective, Move, SchnapsenTrickScorer, GamePhase, RegularMove
...
class BotC(Bot):
    ...
    def get_move(self, perspective: PlayerPerspective, leader_move: Optional[Move], ) -> Move:
        ...
        non_trumps_sorted = sorted(non_trump_suit_moves, key=lambda move: rank_to_points(move.cards[0].rank))   
        ...
        if not perspective.am_i_leader():
            ...
            if perspective.get_talon_size() == 2: 
                for move in non_trumps_sorted:
                    if rank_to_points(move.card.rank) < rank_to_points(leader_move.cards[0].rank):
                        return move
